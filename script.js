document.addEventListener('DOMContentLoaded', function() {
    ...
    async function saveVoteToFirebase(pollId, voterName, optionIndex) {
        try {
            ...
            console.log('Option index type:', typeof optionIndex);
            console.log('Options array length:', pollData.options ? pollData.options.length : 'options is null/undefined');
            
            // Validate pollData.options exists
            if (!pollData.options || !Array.isArray(pollData.options)) {
                throw new Error('Poll options not found or not an array');
            }
            
            // Validate optionIndex
            if (optionIndex >= pollData.options.length || optionIndex < 0) {
                throw new Error(`Invalid option index. Index: ${optionIndex}, Options length: ${pollData.options.length}`);
            }
            
            // Check if the specific option exists
            ...
            // Ensure the option object has all required properties
            if (typeof pollData.options[optionIndex] !== 'object') {
                throw new Error(`Option at index ${optionIndex} is not an object`);
            }
    ...
    document.addEventListener('click', function(e) {
        if (e.target.classList.contains('submit-vote') || e.target.closest('.submit-vote')) {
            ...
            console.log('Poll element:', pollElement);
            ...
            console.log('Option index type:', typeof optionIndex);
            console.log('Is NaN?:', isNaN(optionIndex));
            
            // Validate optionIndex
            if (isNaN(optionIndex) || optionIndex < 0) {
                showToast("Error: Índice de opción inválido", "error");
                console.error('Invalid option index calculated:', optionIndex);
                return;
            }
